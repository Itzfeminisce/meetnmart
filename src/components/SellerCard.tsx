...
export const SellerCard = memo(({
  ...
  // Memoized color class for distance
  const distanceColor = useMemo(() => {
    if (seller.distance_km < 5) return 'text-green-600';
    if (seller.distance_km <= 20) return 'text-yellow-600';
    return 'text-red-600';
  }, [seller.distance_km]);

  // Memoized color class for response time
  const responseTimeColor = useMemo(() => {
    if (seller.avg_response_time_minutes < 10) return 'text-green-600';
    if (seller.avg_response_time_minutes <= 30) return 'text-yellow-600';
    return 'text-red-600';
  }, [seller.avg_response_time_minutes]);

  // Memoized color class for verification
  const verifiedColor = useMemo(() => {
    return seller.seller_status.is_verified ? 'text-green-600' : 'text-gray-400';
  }, [seller.seller_status.is_verified]);
  ...
  return (
    <div className="glass-morphism rounded-xl p-4 sm:p-5 relative overflow-hidden group hover:shadow-lg transition-all duration-300">
      ...
      <div className="my-4" onClick={() => onOpenDetails(seller, 'catalog')}>
     <ImageGridView images={seller.products.items.map(it => it.image)} itemId={seller.seller_id} direction='horizontal'/>
     </div>

      <div className="flex flex-col sm:flex-row flex-wrap items-start sm:items-center gap-2 text-xs text-muted-foreground mb-4">
        <div className={cn("flex items-center", distanceColor)}>
          <MapPin size={12} className="mr-1" />
          <span>{seller.distance_km}km away</span>
        </div>
        <div    className={cn("flex items-center", responseTimeColor)}>
          <Clock size={12} className="mr-1" />
          <span>Responds in ~{Math.round(seller.avg_response_time_minutes)} mins</span>
        </div>
        <div    className={cn("flex items-center", verifiedColor)}>
