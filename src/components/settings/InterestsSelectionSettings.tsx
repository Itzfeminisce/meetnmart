...
import { cacheKeys, useCreateInterests, useGetCategories, useGetInterests } from '@/hooks/api-hooks';
...
const InterestsSelectionSettings = () => {
    // const { updateOnboardingStep } = useAuth()
    const { data: interests, error: _error, isLoading } = useGetInterests()
    ...
    useEffect(() => {
        if (interests && interests.length > 0) {
            setSelectedInterests(interests.filter(it => it.is_interested).map(it => it.id));
        }
    }, [interests])
...
export {InterestsSelectionSettings};