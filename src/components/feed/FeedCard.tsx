  ...
  return (
    <div className="space-y-2" ref={elementRef}>
      ...
      <ShareDialog
        ...
        url={`${location.origin}/feeds/${item.id}/${sluggify(item.title)}`}
        title={item.title}
...
//       })
//     },
//     id: item.id, // Pass the item ID for session storage tracking
//   });

//   // Merge default config with provided config
//   const { contentCharLimit, collapsedLines } = { ...DEFAULT_CONFIG, ...config };

//   const handleCall = () => onCall?.(item.id);
//   const handleMessage = () => {
//     // feedsInteraction.mutate({
//     //   feed_id: item.id,
//     //   type: "message",
//     // })
//   }
//   const handleDelivery = () => onDelivery?.(item.id);
...
//             size="sm"
//             variant="outline"
//             className={`h-8 w-8 p-0 ${item.interactions.items.find(it => it.type == "bookmark" && it.author.id == profile.id) ? 'text-primary' : ''}`}
//             onClick={handleSave}
//           >
//             <Bookmark className={`w-3 h-3 ${item.interactions.items.find(it => it.type == "bookmark" && it.author.id == profile.id) ? 'fill-current' : ''}`} />
//           </Button>
//         </TooltipTrigger>
//         <TooltipContent>
//           <p>{feedsStore.bookmarkedFeeds.has(item.id) ? 'Saved' : 'Save for later'}</p>
//         </TooltipContent>
//       </Tooltip>
...
//   return (
//     <div className="space-y-2" ref={elementRef}>
//       <Card className={getCardStyle(item.type, item.created_at)}>
//         <CardContent className="p-4">
//           <div className="flex flex-col gap-4">
//             {/* Header Section */}
//             <div className="flex items-start justify-between gap-4">
...
//                   <div 
//                     className="w-1.5 h-1.5 bg-gray-100 dark:bg-gray-800 rounded-full flex-shrink-0" 
//                     title="AI Generated" 
//                   />
//                 )} */}
//               </div>
...
//                     <div
//                       key={index}
//                       className="relative flex-shrink-0 w-24 h-24 rounded-lg overflow-hiddensnap-start"
//                     >
//                       <img
//                         src={photo}
//                         alt={`Photo ${index + 1}`}
...
//                 <div className="flex items-center space-x-1">
//                   <Clock className="w-3 h-3" />
//                   <span className=' capitalize'>{formatTimeAgo(item.created_at)}</span>
//                 </div>
//               </div>
...
//                 <div className="flex items-center space-x-1">
//                   <Eye className="w-3 h-3" />
//                   <span>{feedsStore.getFeedStats(item.id).views || 0}</span>
//                 </div>
//                 <div className="flex items-center space-x-1">
//                   <Bookmark className="w-3 h-3" />
//                   <span>{feedsStore.getFeedStats(item.id).bookmarks || 0}</span>
//                 </div>
//                 <div className="flex items-center space-x-1">
//                   <MessageCircle className="w-3 h-3" />
//                   <span>{feedsStore.getFeedStats(item.id).comments || 0}</span>
//                 </div>
//                 <div className="flex items-center space-x-1">
//                   <Phone className="w-3 h-3" />
//                   <span>{0}</span>
