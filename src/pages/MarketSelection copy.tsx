...
import { useFetch, useMutate, queryClient, useGetMarketRecentVisits } from '@/hooks/api-hooks';
...
const MarketSelection = () => {
  ...
  } = useGetMarketRecentVisits({ limit: 3, userId: user.id })
      ...
      {isAuthenticated && recentVisits.length > 0 && (
        <div className="space-y-4 mb-6">
          ...
          {loadingRecent ? (
            ...
            <div className="flex gap-4 overflow-x-auto snap-x snap-center snap-mandatory px-1 scrollbar-small pb-4">
            {recentVisits.map(market => (
              <div
                key={market.id}
                className="glass-morphism rounded-lg p-3 flex items-center card-hover cursor-pointer flex-shrink-0 w-[20rem]"
                onClick={() => handleSelectMarket(market)}
              >
                <Avatar className="w-14 h-14 mr-4 flex-shrink-0">
                ...
                <div className="flex-grow overflow-hidden mr-2 min-w-0">
                  <h3 className="font-medium truncate text-ellipsis">
                    {market.name}
                  </h3>
